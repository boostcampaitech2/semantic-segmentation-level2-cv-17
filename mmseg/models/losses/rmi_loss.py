...
class RMILoss(nn.Module):
	...
	def __init__(self,
					num_classes=11,
					rmi_radius=3, # The square radius of rmi [1, 3, 5, 7], they have a center
					rmi_pool_way=1, # 1 - avg pool, 0 - max pool
					rmi_pool_size=2, # The pool size of the pool operation before calculate RMI loss
					rmi_pool_stride=2, # The pool stride of the pool operation before calculate RMI loss
	...
	def forward(self, logits_4D, labels_4D, weight=None, avg_factor=None, reduction_override=None, **kwargs):
	...
	def forward_sigmoid(self, logits_4D, labels_4D):
		...
		bce_loss_weight = self.weight_lambda * 0.75
		rmi_loss_weight = self.weight_lambda - bce_loss_weight
		# add together
		final_loss = (self.weight_lambda * bce_loss + rmi_loss * (1 - self.weight_lambda) if self.lambda_way
				else bce_loss * bce_loss_weight + rmi_loss * rmi_loss_weight)
