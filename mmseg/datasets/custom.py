...
class CustomDataset(Dataset):
    ...
    def load_annotations(self, img_dir, img_suffix, ann_dir, seg_map_suffix,
        ...
        if split is not None:
            ...
            # img_infos = sorted(img_infos, key=lambda x: int(x['filename'][4:].split('.')[0]))
            img_infos = sorted(img_infos, key=lambda x: int(x['filename'].split('/')[-1].split('.')[0]))
